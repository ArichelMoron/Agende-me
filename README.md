...
```text
app/src/main/java/com/arichelmoron/agende_me/
├── MainActivity.kt                 Entrada da Activity e do Compose
├── App.kt                          Estado da aplicação, abas e fluxos principais
├── components/                     Cards, cabeçalho, FAB e painel de criação
├── network/WeatherRepository.kt    Open-Meteo, mapeamento WMO e cache de ícones
├── notifications/                  Canais, alarmes e receiver de notificações
├── utils/CalendarHelper.kt         Leitura, criação e exclusão na agenda Android
├── utils/AnnotationStorage.kt      Persistência das anotações locais
├── utils/DeletedScheduleStorage.kt Controle de eventos removidos
├── widget/                         Glance widget e armazenamento dos eventos
└── ui/theme/                       Cores, tipografia e tema visual
```
...
3. Conecte um dispositivo ou inicie um emulador com Android API 24 ou superior.
4. Execute a configuração `app`.
5. Conceda as permissões de calendário e notificações solicitadas.

Pela linha de comando, com o JDK configurado:
...
A variante release utiliza minificação e redução de recursos. As regras específicas de R8 ficam em `app/src/main/keepRules/rules.keep` e preservam os contratos usados por Retrofit, Gson e pelo widget.
